...
4.) Run the .exe or .ahk file, it will build the settings.cfg inside the same folder. <br /> To run at startup, make a Windows Task Scheduler task that runs either the script (or .exe) at Windows logon. If you have an issue with the task not launching the .ahk file correctly, just make a batch file in the same directory as the script's .ahk file and set it to launch the script, then direct the logon task at the batch file instead. <br /> <br /> <br />
